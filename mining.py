...
def surface_neigbhours2DTest():
    ...
    loc = (1, 0)
    actualNeighbours = Mine.surface_neigbhours(mine, loc)
...
def is_dangerousTest():
    input = np.array([[-2, -1, 10], [-1, 20, 7], [-1, -1, -1]])
    mine = Mine(underground=input, dig_tolerance=1)
...
def compareTest():
    ...
    for _ in range(numTries):
        ...
        mine = Mine(underground, digTolerance)

        dpResult = search_dp_dig_plan(mine)
        bbResults = search_bb_dig_plan(mine)
...
def hardCodedBBMineTests():
    mines = []

    mines.append(np.array([[-1, -200, 1], [5, 8, 5]]))
    mines.append(np.array([[-1, -1, -1], [-1, 4, -1], [-1, -10, 11]]))
    mines.append(np.array([[-1, -1, 10, 5], [-1, -20, -4, -7], [-1, -1, -1, -21]]))
    mines.append(np.array([[1, 4], [2, 5], [3, 6]]))
    mines.append(np.array([[1, 4, 1, 1], [2, 5, 1, 1], [3, 6, 1, -1]]))
    mines.append(np.array([[1, -6, 1, 1], [2, 5, 1, 1], [3, 6, 1, 1], [3, 6, 1, -10]]))
    mines.append(np.array([[[1, 4, 1, 1], [2, 5, 1, 1], [3, 6, 1, 1]], mines[-1-1] - 1]))

    expectedResults = [12, 2, 0, 21, 26, 28, 42]


    for i,m in enumerate(mines):
        mine = Mine(m, dig_tolerance=1)

        # print(search_bb_dig_plan(mine)[0])
        bbResult = search_bb_dig_plan(mine)

        assert bbResult[0] == expectedResults[i]
    #end
#end

def hardCodedDPMineTests():
    mines = []
